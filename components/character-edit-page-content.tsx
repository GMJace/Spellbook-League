...
export async function CharacterEditContent({
  params,
  searchParams,
  admin = false,
}: {
  ...
  const user = admin ? await requireAdminUser() : await requireRole("PLAYER");
  ...
  return (
    <main className="stack character-workflow-page">
      <section className="panel ledger-panel stack">
        ...
        >
          <div>
          <p className="eyebrow">{admin ? "Admin · Edit character log" : "Edit character log"}</p>
          <h1>{character.name}</h1>
          </div>
          <Link className="button button-secondary" href={admin ? "/admin/characters" : `/player/characters/${character.id}`}>
            Back
          </Link>
        </div>
      </section>
