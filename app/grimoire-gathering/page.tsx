...
export default async function GrimoireGatheringPage({
  ...
  const discordStatusMessageMap: Record<string, string> = {
    invalid: "That password did not match the Grimoire Gathering Discord access code.",
    unlocked: "Discord unlocked.",
  };
  const discordStatusMessage = params.discord
    ? discordStatusMessageMap[params.discord] ?? ""
    : "";
  const isDiscordUnlocked =
    cookieStore.get(GRIMOIRE_DISCORD_COOKIE_NAME)?.value ===
    createGrimoireDiscordAccessToken(discordSettings.password);
      ...
      <section className="homepage-community-row ggcon-community-row">
        <section className="card ledger-panel stack ggcon-community-card-half">
          ...
          {discordStatusMessage ? (
            <p style={{ color: "#ffffff", margin: 0 }}>{discordStatusMessage}</p>
          ) : null}
          {isDiscordUnlocked ? (
            <div className="inline-actions">
              <a
                className="button ggcon-discord-link"
                href={discordSettings.inviteUrl}
                target="_blank"
                rel="noreferrer"
              >
                <svg
            ...
            <form action={unlockGrimoireDiscord} className="form-stack">
              <label>
                Discord password
                <input
